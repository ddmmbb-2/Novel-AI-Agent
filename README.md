...
## 🖼️ 系統介面說明 / UI Overview

* **🤖 系統狀態與日誌 (左側)**：即時顯示 AI 的思考邏輯，包含劇情構思、文筆潤飾過程，以及最核心的 **XML 三維記憶庫更新**（包含設定、事件、伏筆與全局摘要）。
* **📜 章節清單 (中間)**：直觀管理所有已創作的章節，並可在此選定特定章節執行「時光倒流」功能，系統會自動回復該時間點的記憶快照。
* **📖 繁體預覽區 (右側)**：即時呈現 AI 最終生成的小說正文，採用適合閱讀的繁體中文與字體排版。
* **⚙️ 自動化控制台 (下方)**：提供一鍵「開始自動創作」與「暫停」功能，並具備「機器散熱倒數」狀態顯示，確保系統穩定運行並即時同步至雲端。

![系統介面預覽](1773223903684.jpg)
---
